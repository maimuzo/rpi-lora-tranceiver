...
bool isReceiveFirstPacket = false;
...
void sendMode(char *text){
    ...
    long startTime = getEpochTime();
    for(int i=0; i<5; i++){
        txlora(hello, strlen((char *)hello));
        delay(5000);
    }
    long endTime = getEpochTime();
...
void receiveMode(){
    ...
    // wait delay
    while(endTime < startTime + waitSec) {
        receivepacket(); 
        delay(1);
